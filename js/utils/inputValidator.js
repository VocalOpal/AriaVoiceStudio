export class InputValidator {
        ...
        switch (type) {
            ...
            case 'css':
                return input
                    ...
                    .replace(/@import/i, '')
                    .replace(/<script/i, '')
                    .substring(0, this.limits.cssMaxLength);
    ...
    static validate(input, type, options = {}) {
        ...
        switch (type) {
            ...
            case 'pitchRange':
                const parts = String(sanitized).split('-').map(p => parseInt(p.trim(), 10));
                const [min, max] = parts;
            ...
            case 'css':
                ...
                if (/@import|javascript:|expression\(/i.test(sanitized)) {
            ...
            case 'file':
                if (!input || !input.name) errors.push('Invalid file');
                    ...
                    if (input.size > this.limits.maxFileSize) errors.push('File is too large (max 10MB)');
     ...
     * @param {Object} schema - Validation schema { field: { type, required, options } }
     * @returns {Object} { isValid, results, errors }
     */
